...
import { CONFIG, HK_TIMEZONE, HK_UTC_OFFSET_MINUTES, MS_PER_MINUTE, MS_PER_DAY, DEFAULT_HSI_CODES, HOLIDAY_CACHE_KEY, HOLIDAY_CACHE_TTL_MS, FALLBACK_HK_HOLIDAYS_BY_YEAR } from './config.js';
import * as storage from './storage.js';
import * as api from './api.js';
import * as ui from './ui.js';

// Re-export for backward compatibility
const { DEFAULT_CODES, HSI_LIST_URL, STORAGE_KEYS } = CONFIG;
const { HSI_LIST: STORAGE_KEY_HSI_LIST } = STORAGE_KEYS;
...
// HOLIDAY_CACHE_KEY, HOLIDAY_CACHE_TTL_MS, and FALLBACK_HK_HOLIDAYS_BY_YEAR 
...
function getHongKongShiftedDate(date = new Date()) {
    return new Date(date.getTime() + HK_UTC_OFFSET_MINUTES * MS_PER_MINUTE);
}

function getHongKongParts(date = new Date()) {
...
function createUtcDateFromHongKongParts(year, month, day, hour = 0, minute = 0, second = 0, ms = 0) {
    return new Date(Date.UTC(year, month, day, hour, minute, second, ms) - HK_UTC_OFFSET_MINUTES * MS_PER_MINUTE);
}

function formatHongKongTime(date = new Date(), options = {}) {
...
function getTradingDayStart(date) {
    const parts = getHongKongParts(date);
    return createUtcDateFromHongKongParts(parts.year, parts.month, parts.day, TRADE_START_HOUR, TRADE_START_MIN, 0, 0);
}

function getNextTradingDayStart(fromDate) {
    const base = getTradingDayStart(fromDate);
    let target = new Date(base.getTime() + MS_PER_DAY);
    while (!isWeekday(target) || isHolidayDate(target)) {
        target = new Date(target.getTime() + MS_PER_DAY);
    }
    return target;
}

function getMarketStatus(now = new Date()) {
    const hkNow = getHongKongParts(now);
    const minutesNow = hkNow.hour * 60 + hkNow.minute;
    ...
    if (minutesNow >= morningEnd && minutesNow < afternoonStart) {
        const nextOpen = createUtcDateFromHongKongParts(
            hkNow.year,
            hkNow.month,
            hkNow.day,
            TRADE_AFTERNOON_START_HOUR,
            TRADE_AFTERNOON_START_MIN,
            0,
            0
        );
...
function scheduleNextRefresh() {
    ...
    if (marketStatus.state === 'open') {
        ...
        nextRefreshEl.textContent = formatHongKongTime(nextTime, {
            hour: '2-digit',
            minute: '2-digit',
            second: '2-digit'
        }) + ' HKT';
    ...
    const dateStr = formatHongKongTime(nextStart, {
        year: 'numeric',
        month: '2-digit',
        day: '2-digit',
        hour: '2-digit',
    ...
    nextRefreshEl.textContent = `${dateStr} HKT`;
...
function getRowForStock(code) {
    ...
    row.addEventListener('touchstart', (e) => {
        if (isHsiMode()) return; // Disable swipe in HSI mode
    ...
    row.addEventListener('touchmove', (e) => {
        ...
        if (diff < 0 && diff > -200) {
            ...
            // Show delete hint background
            if (diff < -SWIPE_THRESHOLD / 2) {
                row.style.background = 'linear-gradient(to right, #fff5f5 0%, #fff5f5 30%, transparent 100%)';
            }
        }
    }, { passive: true });
    ...
    row.addEventListener('touchend', () => {
        ...
        if (diff <= -SWIPE_THRESHOLD && !isHsiMode()) {
            // Swiped enough - confirm delete
            if (confirm(`確定要刪除股票 ${code.replace(/^0+/, '')} 嗎？`)) {
                removeStock(code);
                showToast(`已刪除股票 ${code.replace(/^0+/, '')}`, 1500);
            } else {
                // Reset position
...
async function updateStockTable() {
    ...
    try {
        ...
        dom.currentTime.textContent = `HK ${formatHongKongTime(new Date(), {
            hour: '2-digit',
            minute: '2-digit',
            second: '2-digit'
        })}`;
...
function debouncedSaveColumnSettings() {
    if (saveColumnTimeout) clearTimeout(saveColumnTimeout);
    saveColumnTimeout = setTimeout(saveColumnSettings, 300);
}
...
function initPullToRefresh() {
    ...
    container.addEventListener('touchmove', (e) => {
        ...
        if (diff > 0 && diff < PULL_THRESHOLD * 2) {
            indicator.style.opacity = Math.min(diff / PULL_THRESHOLD, 1).toString();
            indicator.style.top = `${Math.min(diff / 2 - 50, 10)}px`;
...
document.addEventListener('DOMContentLoaded', async () => {
    try {
        ...
        if (error.message && (error.message.includes('CORS') || error.message.includes('cross-origin') || error.message.includes('Failed to fetch'))) {
            if (dom.tbody) {
                dom.tbody.innerHTML = `
                    <tr>
                        <td colspan="12" style="text-align: center; padding: 20px; color: #d9534f;">
                            <strong>⚠️ CORS 錯誤 / 網絡連接問題</strong><br>
                            <span style="color: #666; font-size: 13px;">
                                無法連接到數據 API。這可能是因為：<br>
                                1. 網絡連接問題<br>
                                2. API 服務暫時不可用<br>
                                3. 瀏覽器安全限制<br><br>
                                請檢查網絡連接並重新整理頁面。
            ...
            if (dom.status) {
                dom.status.textContent = '連接失敗 - 檢查網絡';
                dom.status.style.color = '#d9534f';
            }
        }
    }
});
